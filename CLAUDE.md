...
- **`Sync\*`** — multisite consolidation roll-up (subsite WP ERP books → Holding site, blog 1). `EntityMap` owns the `blog_id → "NN"` two-digit entity code map (network option `erp_budget_entity_map`, baked default = the live unilorinholdings.com network). `LedgerSync` mirrors every entity's *chart of accounts* into Holding as `NN-<code>` twin ledgers (Water's `1341` → `09-1341`; Holding's own `1211` → `01-1211`), widening Holding's `erp_acct_ledgers.code` to VARCHAR on first run; it's a reconcile (no WP ERP hook on ledger CRUD) — creates missing twins, renames on source rename, keeps (and logs) orphan twins when a source ledger is deleted, skips archived (`unused=1`) ledgers. `ConsolidationSync` mirrors GL *postings*: one source `trn_no` → synthetic `trn_no = blog*1e6 + trn_no` (`SYNTHETIC_TRN_BASE`) in Holding's `erp_acct_ledger_details`, each line remapped to its twin via `LedgerResolver`; state/idempotency in `erp_budget_sync_map` on Holding (`Migration::ensureSyncTable()`). Holding (entity 01) is **not** consolidated into its own books: `ConsolidationSync::runBlog()` returns early for the Holding blog (after the LedgerSync pass), because Holding's staff post its own journals straight onto the `01-` twin ledgers — those accounts already carry Holding's actuals, and mirroring would only try to remap `01-5020` → `01-01-5020` and block. Consolidated views scope to `NN-`-prefixed ledgers, so Holding is still counted exactly once. Filter `erp_budget_consolidate_holding_into_self` (default false) restores self-consolidation, but then Holding staff must post to the un-prefixed ledgers instead. `LedgerSync` skips its orphan check for the Holding blog (every `01-` twin's un-prefixed source is deleted by design). `LedgerSync` runs first on every sweep so new ledgers don't leave postings `blocked`; the source read still filters out `trn_no >= SYNTHETIC_TRN_BASE` so a self-consolidation sweep can't re-mirror its own rows. `SyncCron` = hourly authoritative `runAll()` (Holding cron only); `SyncListener` = ~1-min single-blog nudge on `erp_acct_new_transaction_*` (fires for Holding too); `SyncRestController` = the `/erp/v1/consolidation/*` routes. `LedgerDeleteBridge` rides the `rest_request_before/after_callbacks` filters on WP ERP's own `DELETE /erp/v1/accounting/v1/ledgers/{id}` (WP ERP fires no hook) to propagate a ledger deletion to its peer — plain subsite code ⇄ Holding `NN-` twin, plain Holding code ⇄ `01-` twin, both directions — and **blocks the whole delete with a 409** (WP ERP's delete included) if the ledger or its peer has any `erp_acct_ledger_details`/`erp_acct_opening_balances` rows. Disable via filter `erp_budget_sync_propagate_ledger_delete`. Entity name prefixes for twins come from network option `erp_budget_entity_labels` (`[blog_id => "Water"]`), else an existing twin's prefix, else the blog name minus `Unilorin`.
...
POST   /erp/v1/consolidation/sync            { blog_id? }   # ledgers + postings sweep
POST   /erp/v1/consolidation/sync-ledgers    { blog_id? }   # chart of accounts only
GET    /erp/v1/consolidation/status
GET    /erp/v1/consolidation/entity-map
POST   /erp/v1/consolidation/entity-map      { "2":"02", ... }
```
